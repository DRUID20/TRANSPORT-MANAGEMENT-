...
function toBooking(r: Row): Booking {
  return {
    ...
    agreedAmount: Number(r.agreedAmount),
    agreedBasis: r.agreedBasis as RateBasis,
    agreedCurrency: r.agreedCurrency as Currency,
...
export async function createBooking(input: NewBooking): Promise<Booking> {
  ...
  const rows = await db
    ...
    .values({
      ...
      agreedAmount: String(input.agreedAmount),
      agreedBasis: input.agreedBasis,
      agreedCurrency: input.agreedCurrency,
      status: "draft",
    })
    .returning();
