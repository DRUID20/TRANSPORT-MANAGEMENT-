...
import { cancelBill, payBill, postBill } from "@/server/actions/ap";
...
export function BillActions({
  ...
  function cancel() {
    if (!confirm("Cancel this bill? If posted, the GL entry will be reversed.")) return;
    setError(null);
    start(async () => {
    ...
    return (
      <Card>
        <CardContent className="!p-4 text-sm text-fg-tertiary">
          Bill cancelled — no further actions.
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {status === "draft"
            ? "Post the bill to the GL (Dr Expenses / Cr AP)."
            : "Pay the supplier. Posts Dr AP / Cr Bank automatically."}
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {status === "draft" && (
            <>
              <Button onClick={post} disabled={pending} variant="primary">
