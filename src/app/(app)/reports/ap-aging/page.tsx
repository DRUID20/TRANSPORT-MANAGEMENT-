...
import { apAgingBySupplier } from "@/server/actions/reports";
...
export default async function ApAgingPage({
  ...
  const rows = await apAgingBySupplier(today);
  const totals = rows.reduce(
    ...
    { current: 0, d1to30: 0, d31to60: 0, d61to90: 0, d90plus: 0, total: 0, billCount: 0 },
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        eyebrow="Finance · AP"
        title="AP Aging"
        description={`Open supplier balances aged into buckets as of ${today}. KES base.`}
        actions={
          <ReportExportMenu exportPath={`/api/reports/ap-aging/export?asOf=${today}`} />
        }
      />

      <ReportLetterhead title="Accounts Payable — Aging" period={`As at ${today}`} />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {rows.map((r) => (
                  <tr key={r.supplierId} className="transition-colors hover:bg-bg-base/40">
                    <td className="px-5 py-2 text-fg-primary">{r.supplierName}</td>
                    <td className="px-3 py-2 text-right font-mono tnum text-fg-secondary">{r.billCount}</td>
