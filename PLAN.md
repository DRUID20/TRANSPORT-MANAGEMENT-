...
- 2026-05-11: **SCOPE LOCK — fuel-only TMS.** This system transports PMS (petrol) and AGO (diesel) only. General-cargo abstractions removed from new flows but kept as legacy types so existing seeds still validate. Rework split into:
  ...
  - **F-6 (shipped)** — fuel-haul performance surface. `TruckScorecard` gains four new fields (`loadedLitres`, `kesPerLoadedLitre`, `avgUllagePct`, `tripsWithUllage`) computed from captured loading + discharge observations on the truck's trips in the range, preferring `loadedLitres20C` so thermal contraction doesn't muddy the KES-per-L figure. `/tracker/[truckId]` grid widened to 6 columns and now renders two new KpiCards: **KES per loaded litre** (green ≥ 8, warning 6-8, red < 6) and **Avg ullage** (green within 0.5% threshold, red above). New `/reports/ullage` page lists every trip with both ends captured, sorted worst-loss-first, with per-trip badges and a 5-tile summary (trips measured / above threshold / total loaded / total delivered / weighted-average fleet ullage). Listed even when below threshold so the volumetric accountant can spot a consistent +0.3% pattern across one driver before it crosses the alert line. Two demo trips seeded with realistic AGO + PMS loading sheets (one within threshold, one above) so the report and KPIs render data on first boot.
  - Fuel-only TMS rework is **complete**. Next moves are operator-driven: capture a few real loads, tune the ullage alert threshold if 0.5% is wrong for your sites, and decide whether to fold the legacy non-fuel rate seeds into the new per-litre cards.
